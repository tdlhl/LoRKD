...
class Med_SAM_Dataset_downstream(Dataset):
    def __init__(self, 
                 jsonl_file, 
                 dataset_config,
        ...
        for sample in lines:
            ...
            if 'is_3D' not in sample or sample['is_3D']=='3D':
                for i in range(round(repeat_times)):
                    ...
                    # if sample['dataset'] in ['TotalSegmentator_Muscles_upper limb', 'TotalSegmentator_Muscles_lower limb', 
                    # 'DAP_Atlas_lower limb', 'DAP_Atlas_upper limb', 'WORD_lower limb']:
                    #     print('Sample belongs to UL/LL, weight=', weight)
                    #     print('[sampling_weight] in dataset_config=', self.dataset_config[sample['dataset']]['sampling_weight'])
