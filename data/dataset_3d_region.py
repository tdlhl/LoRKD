        ...
        while True:
            with self.lock:
                try: 
                    ...
                    if self.query_counter > self.region_change_interval:
                        self.query_counter = 1
                        self.current_region_index = self.select_region()
                        if self.current_region_index==-1:
                            raise ValueError("select_region() returns -1")
                        print('Using region', self.current_region_index)
