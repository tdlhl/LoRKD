...
class Splited_Images_and_Labels_3D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        error_log_path = '/mnt/petrelfs/lihaolin/project/SAT-decompose/SAT-my/log/nano_eval/error/error_log_33_my.txt'  # 定义错误日志文件的路径
        # divide patches into batches
        # patches = [torch.tensor(np.load(p)) for p in datum['patch_path']]
        patches = []
        for p in datum['patch_path']:
            try:
                patch = torch.tensor(np.load(p))
                patches.append(patch)
            except Exception as e:  # 捕获所有异常，你也可以只捕获ValueError或具体的异常类型
                error_message = f"Error loading patch {p}: {e}\n"
                print(error_message.strip())  # 打印错误信息
                with open(error_log_path, 'a') as error_log:  # 'a'模式表示追加，这样不会覆盖之前的错误信息
                    error_log.write(error_message)  # 将错误信息写入文件
        ...
        for mask_path, y1x1z1_y2x2z2 in zip(mask_paths, y1x1z1_y2x2z2_ls):
            ...
            if y1x1z1_y2x2z2 != False:
                ...
                try:
                    ...
                    error_message = f"Error loading mask {mask_path}: {e}\n"
                    print(error_message.strip())  # 打印错误信息
                    with open(error_log_path, 'a') as error_log:  # 'a'模式表示追加，这样不会覆盖之前的错误信息
                        error_log.write(error_message)  # 将错误信息写入文件
                    # mask已经初始化为全零，所以这里不需要再设置或跳过
