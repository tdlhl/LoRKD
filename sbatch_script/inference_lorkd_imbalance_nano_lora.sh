...
srun -p medai_llm --quotatype=reserved \
--async -o /mnt/petrelfs/lihaolin/project/SAT-decompose/LoRKD-seg/log/async/inference/lorkd_imbalance_nano_inference_demo.out \
--cpus-per-task=16 \
--gres=gpu:1 torchrun \
--nnodes 1 \
--nproc_per_node 1 \
--rdzv_id 100 \
--rdzv_backend c10d \
--rdzv_endpoint=127.0.0.1:28361 inference_lora_imbalance.py \
--rcd_dir '/mnt/petrelfs/lihaolin/project/SAT-decompose/LoRKD-seg/log/inference/lorkd_imbalance_nano_inference_demo' \
...
--vision_backbone 'UNET_lora' \
--checkpoint '/mnt/petrelfs/lihaolin/model/SAT/decompose/49_lora/lorkd-imbalance-nano.pth' \
--partial_load True \
--biolord_checkpoint '/mnt/hwfile/medai/lihaolin/models/huggingface/BioLORD-2023-C' \
--knowledge_encoder_checkpoint '/mnt/petrelfs/lihaolin/model/SAT/SAT_ckpt/nano_text_encoder.pth' \
...
--batchsize_3d 1 \
--max_queries 256 \
--gamma_list "12,10,10,4,8,8,12,2" \
--num_workers 8 