...
class ConvDropoutNormReLU_lora(nn.Module):
    def __init__(self,
                 conv_op: Type[_ConvNd],
                 input_channels: int,
                 output_channels: int,
        ...
        super(ConvDropoutNormReLU_lora, self).__init__()
        self.input_channels = input_channels
        self.output_channels = output_channels
        stride = maybe_convert_scalar_to_list(conv_op, stride)
        self.stride = stride
    ...
    def forward(self, x, task_labels):
        if task_labels == None:
            print('task_labels == None! using only the backbone')
            out = self.conv(x)
            out = self.norm(out)
            out = self.nonlin(out)
            return out
        # print('covdroprelu里task_labels=', task_labels)
        out = self.conv(x, task_labels)
        # print('过了lora_conv')
...
class StackedConvBlocks_lora(nn.Module):
    def __init__(self,
                 num_convs: int,
                 conv_op: Type[_ConvNd],
                 input_channels: int,
        ...
        stages = []
        stages.append(ConvDropoutNormReLU_lora(
                conv_op, input_channels, output_channels[0], kernel_size, initial_stride, conv_bias, norm_op,
                norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs, nonlin_first
            ))
        for i in range(1, num_convs):
    ...
    def forward(self, input_x):
        x, task_labels = input_x[0], input_x[1]
        # print('stacksconv_blocks里task_labels=', task_labels)
        for s in range(len(self.stages)):
            x = self.stages[s](x, task_labels)
        return x
