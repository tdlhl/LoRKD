...
from dynamic_network_architectures.building_blocks.plain_conv_encoder import PlainConvEncoder, PlainConvEncoder_lora
from dynamic_network_architectures.building_blocks.unet_decoder import UNetDecoder, UNetDecoder_Seg, UNetDecoder_lora
...
class PlainConvUNet_lora(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
        ...
        assert len(n_conv_per_stage_decoder) == (n_stages - 1), "n_conv_per_stage_decoder must have one less entries " \
                                                                ...
                                                                f"stages, so it should have {n_stages - 1} entries. " \
                                                                f"n_conv_per_stage_decoder: {n_conv_per_stage_decoder}"
        self.encoder = PlainConvEncoder_lora(input_channels, n_stages, features_per_stage, conv_op, kernel_sizes, strides,
                                        n_conv_per_stage, conv_bias, norm_op, norm_op_kwargs, dropout_op,
                                        dropout_op_kwargs, nonlin, nonlin_kwargs, return_skips=True,
                                        nonlin_first=nonlin_first)
                                       
        self.decoder = UNetDecoder_lora(self.encoder, n_conv_per_stage_decoder, deep_supervision,
                                   nonlin_first=nonlin_first)

    def forward(self, x, task_labels=None):
        # print('unet里task_labels=', task_labels)
        skips = self.encoder(x, task_labels) # [2, 32, 256, 256, 96] ... [2, 768, 8, 8, 3]
        outs = self.decoder(skips, task_labels)  # [2, 32, 256, 256, 96] ... [2, 512, 16, 16, 6]
        return skips, outs   # latent_embeddings(a list of multiscale features), perpixel_embeddings(a list of decoder outputs)
...
class PlainConvUNet_lora_encoder(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
...
class PlainConvUNet_lora_decoder(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
